...
# 1. SETUP
# Retrieves key from your environment (safer than pasting it here)
...
# 2. CONFIGURATION
# Exact path to your library
BASE_PATH = r"C:\UNT_DEBUGGING\dataset\converted_library\Mathematics"
TARGET_FOLDERS = ["7", "8", "9", "10", "11"]

# Output: Saves to the folder where you run the script (backend folder)
OUTPUT_FILE = os.path.join(os.getcwd(), "data", "math_knowledge_graph.json")

CHUNK_SIZE_CHARS = 15000

# 3. PROMPT (The "Teacher Brain")
SYSTEM_PROMPT = """
You are an expert Curriculum Developer for the Kazakhstan National Testing Center (UNT).
Your task is to analyze an educational Markdown file (in KAZAKH) and extract a Knowledge Graph.

RULES:
1. Identify KEY CONCEPTS (Topics, Formulas, Definitions).
...
5. Use 'id' in English Snake Case (e.g., "quadratic_equation") for linking.
...
def clean_markdown_chatter(text):
    """Removes 'Sure, here is the markdown...' conversational filler from the start."""
    lines = text.split("\n")
    if (
        len(lines) > 0
        and len(lines[0]) < 100
...
def extract_from_chunk_with_retry(text_chunk, chunk_index, filename, retries=3):
    """Tries to extract JSON. If it fails, it tries again up to 3 times."""
...
def read_and_chunk_markdown(file_path, size):
    try:
        ...
        # Fallback for older Windows encoding issues
    ...
    for folder in TARGET_FOLDERS:
        ...
        for file in files:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if "concepts" in data:
                    # Tag metadata so we know where this concept came from
            ...
            print("")  # Empty line for readability between books

    # Save Results
    print(f"\n💾 Saving {len(all_data)} total concepts...")
